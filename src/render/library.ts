 ...
 * is derived, and a recipe that carries none of them still lists by title.
 */
import { html } from "hono/html"
import type { HtmlEscapedString } from "hono/utils/html"
import type { LibraryCardView } from "./view-model.js"
...
const cardItem = (card: LibraryCardView, href: RecipeHref): Fragment => html`<li class="card">${
  card.heroImage === undefined
    ? nothing
    : html`<img src="${card.heroImage.src}" alt="${card.title}">`
}
<h2><a href="${href(card.id)}">${card.title}</a></h2>
