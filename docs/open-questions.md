...
| OQ-47 | whether a path named by an accepted record is checked against the tree | open — `ADR-0025` names `src/http/cooking-app.ts` in its "two callers in the tree" section; the cooking route moved into `src/http/pages-app.ts` and that file was deleted, so the record now cites a path that does not exist and nothing noticed. `CFV1-IDS` made every record *number* resolve, which is why the number-shaped half of this class is closed and the path-shaped half is not. A record is accepted and never rewritten, so a stale path is repaired by a superseding note rather than an edit — which means a guard cannot simply demand that every cited path exist today: it would go red on records that are correct about a tree that has since moved on. What has to be decided is which paths are load-bearing enough to guard (a path in a Decision or a Consequences section, say, but not one inside a quoted diff), and what a violation is told to do about it. Until that is decided no item can be shaped, because the proof's own assertion is the undecided part. Needs a maintainer decision |
