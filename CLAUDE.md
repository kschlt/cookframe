...
**Do this even when `.aos/` already exists.** A container can be reused between sessions, so a
...
There is a `SessionStart` hook in `.claude/settings.json` that runs the same script. **Do not
rely on it.** Measured three times in this account's cloud containers: no hook in that file
fires — not `SessionStart`, not `PreToolUse`, not `PostToolUse` — while `CLAUDE.md` and
`.claude/skills/` load normally in the same session. Until that is understood, this instruction
is the mechanism and the hook is a bonus.
